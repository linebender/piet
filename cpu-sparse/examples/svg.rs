...
use piet_next::peniko::kurbo::{Affine, BezPath, Point, Size, Stroke, Vec2};
        ...
        if node.is_element() {
            ...
            match node.tag_name().name() {
                ...
                "path" => {
                    ...
                    let path: piet_next::Path = bp.into();
...
fn parse_color(color: &str) -> Color {
    ...
    if let Ok(c) = piet_next::peniko::color::parse_color(color) {
        c.to_alpha_color()
    } else {
